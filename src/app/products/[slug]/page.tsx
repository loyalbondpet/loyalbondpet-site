...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${product.name} - LoyalBond`,
      description: product.shortDescription,
      url: `https://loyalbondpet.com/products/${slug}`,
      type: 'website',
    },
  };
}
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const productJsonLd = {
    ...
    brand: { '@type': 'Brand', name: 'LoyalBond' },
    offers: product.sizeVariants
      ? {
          '@type': 'AggregateOffer',
          lowPrice: Math.min(...product.sizeVariants.map(v => v.price)),
          highPrice: Math.max(...product.sizeVariants.map(v => v.price)),
          priceCurrency: 'USD',
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: product.collection, item: `https://loyalbondpet.com/collections/${product.collectionSlug}` },
      { '@type': 'ListItem', position: 3, name: product.name, item: `https://loyalbondpet.com/products/${slug}` },
    ],
  };
